...
with open("/opt/airflow/schemas/earthquake.avsc", "r") as schema_file:
...
def delivery_report(err, msg):
    if err is not None:
        ...
        if partition not in batch_offsets:
            batch_offsets[partition] = {
                "start": offset,
                "end": offset
            }
        else:
            batch_offsets[partition]["end"] = offset

        logger.info(
            f"record {msg.key()} successfully produced"
            f" to {msg.topic()} [{partition}]"
            f" at offset {offset}"
        )
...
data = extract_earthquakes(start_date, end_date)
...
with open("/opt/airflow/data/batch_offsets.json", "w") as f:
    json.dump(batch_offsets, f)